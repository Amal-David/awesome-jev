...
for attempt in 1 2 3; do
  ...
  if git merge-base --is-ancestor HEAD FETCH_HEAD; then
    report "Directory commit already present remotely: $(git rev-parse --short HEAD)"; exit 0
  fi
  if ! git rebase FETCH_HEAD; then
    git rebase --abort || true
    report 'Publish stopped: concurrent changes conflict. No force-push was attempted.'; exit 1
  fi
  ...
  git fetch --no-tags origin "refs/heads/$branch"
  if git merge-base --is-ancestor HEAD FETCH_HEAD; then
    report "Published and verified directory update: $(git rev-parse --short HEAD)"; exit 0
  fi
done
